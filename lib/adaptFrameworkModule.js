...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async initRoutes() {
    ...
    this.rootRouter.addRoute({
      route: '/preview/:id/*',
      handlers: { get: this.getHandler.bind(this) }
    });
  ...
  async postHandler(req, res, next) {
    ...
    try {
      const data = await FWUtils.performCourseAction(req.params.id, action, req.params.id);
      ...
      this.log('error', `failed to ${action} course '${req.params.id}'`);
      next(e);
    }
  }
