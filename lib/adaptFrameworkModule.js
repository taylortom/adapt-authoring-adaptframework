...
class AdaptFrameworkModule extends AbstractModule {
  ...
  */
  async loadSchemas() {
    const [authored, content, jsonschema, tags] = await this.app.waitForModule('authored', 'content', 'jsonschema', 'tags');

    const schemas = await glob(`${this.path}/src/**/schema/*.schema.json`);
    await Promise.all(schemas.map(s => jsonschema.registerSchema(s)));

    await tags.registerModule(content);
    ...
    if(this.app.dependencies.usergroups) {
      ...
      usergroups.registerModule(usergroups);
    }
  }
