...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async loadSchemas() {
    ...
    const schemas = Object.values(adapt_framework.schemas);
    return Promise.all(schemas.map(async s => {
      await jsonschema.registerSchema(s);
      this.contentSchemas.push(s.title);
    }));
  }
  async initRoutes() {
