...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async initFramework() {
    ...
    try {
      ...
      await fs.move(path.join(this.framework_dir, 'adapt-framework.json'), path.join(this.framework_dir, 'adapt.json'));
      return await new Promise((resolve, reject) => {
        const bin = path.join(this.app.getConfig('root_dir'), 'node_modules', 'adapt-cli', 'bin', 'adapt');
        exec(`${bin} install`, { cwd: this.framework_dir }, (error, stdout, stderr) => {
