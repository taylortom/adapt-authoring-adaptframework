...
const zipper = require('adapt-authoring-zipper');
...
class AdaptFrameworkUtils {
  ...
  static async recordBuildAttempt(action, courseId, location) {
  ...
  static async performCourseAction(courseId, action) {
    ...
    if(action === 'preview') location = path.join(buildDir, 'build');
    else if(action === 'publish') zipPath = buildDir;
    else if(action === 'export') zipPath = path.join(buildDir, 'build');
    ...
    const attemptData = await this.recordBuildAttempt(action, courseId, location);
    return attemptData;
  }
