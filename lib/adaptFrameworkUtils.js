  ...
  */
  static async createLocalFramework(cwd) {
    ...
    await Promise.all([
      ...
      this.installFrameworkPlugins(cwd)
    ]);
  }
  static async installFrameworkPlugins(cwd) {
    const contentplugin = await App.instance.waitForModule('contentplugin');
    const adaptJson = await fs.readJSON(`${cwd}/adapt.json`);
    const _install = async deps => {
      if(!deps.length) {
        return;
      }
      try {
        const [name, version] = deps.pop();
    ...
    await _install(Object.entries(adaptJson.dependencies));
  }
  /**
  ...
  */
  ...
  static async handleImportFile(req, res) {
    if(req.get('Content-Type').indexOf('multipart/form-data') === 0) {
      ...
      return new Promise((resolve, reject) => {
        middleware.fileUploadParser('application/zip')(req, res, (error) => {
