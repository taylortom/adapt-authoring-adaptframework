 ...
 */
export default class AdaptFrameworkModule extends AbstractModule {
  /** @override */
  async init() {
    /**
  ...
  async installFramework() {
    try {
      const { installed, compatible } = await AdaptCli.getUpdateInfo('adapt_framework', this.path);
      const shouldUpdate = this.app.args['update-framework'];
      if(shouldUpdate) {
        await fs.remove(this.path);
      } else {
        if(installed) {
          if(semver.gte(installed, compatible)) {
            this.log('debug', `valid local adapt_framework found (${installed}), skipping install`);
          } else {
            this.log('info', `a newer version of the adapt_framework is available (${compatible}), pass the --update-framework flag to update`);
          }
          return;
        } else {
          this.log('debug', `no local adapt_framework found, installing ${compatible}`);
        }
      }    
      ...
      await AdaptCli.installFramework({
        version: compatible,
        dir: this.path,
        repo: this.getConfig('frameworkRepository')
      });
      this.log('debug', 'local adapt_framework installed');
    } catch(e) {
      try { // check if framework already exists, in which case not a fatal error
        await fs.stat(this.path);
        this.log('warn', 'failed to update framework, continuing with existing version');
      } catch(e) {
        throw new Error(`Failed to install framework, ${e}`);
      }
    }
  }
  /**
  ...
  async loadSchemas() {
    ...
    const schemas = await AdaptCli.getSchemas({ dir: this.path });
