...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async init() {
     ...
     * @type {String}
     */
    this.path = this.app.getConfig('frameworkDir');
    /**
  ...
  async installFramework() {
    try {
      ...
      this.log('debug', 'existing local adapt_framework found, skipping install');
    } catch(e) {
      this.log('debug', 'no local adapt_framework found, installing');
      await FWUtils.createLocalFramework(this.path);
      this.log('debug', 'local adapt_framework installing');
    }
  }
  /**
  ...
  async initRoutes() {
    ...
    auth.secureRoute(`${this.apiRouter.path}/preview`, 'post', ['preview:adapt']);
    auth.secureRoute(`${this.apiRouter.path}/publish`, 'get', ['publish:adapt']);
    auth.secureRoute(`${this.apiRouter.path}/publish`, 'post', ['publish:adapt']);
    auth.secureRoute(`${this.apiRouter.path}/import`, 'post', ['import:adapt']);
    auth.secureRoute(`${this.apiRouter.path}/export`, 'get', ['export:adapt']);
    auth.secureRoute(`${this.apiRouter.path}/export`, 'post', ['export:adapt']);
  }
  /**
