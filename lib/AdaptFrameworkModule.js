...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async installFramework() {
    try {
      ...
      try {
        ...
        if(e.code === 'ENOTFOUND') this.log('error', `no network connection`);
        else throw e;
      }
      ...
      try {
        ...
        this.log('debug', `no local adapt_framework found, installing ${latest}`);
      ...
      if(pkg) {
        ...
        if(shouldUpdate) {
          ...
          this.log('debug', `local adapt_framework v${pkg.version} installed`);
  ...
  async loadSchemas() {
    ...
    const schemas = await AdaptCli.getSchemaPaths({ cwd: this.path });
