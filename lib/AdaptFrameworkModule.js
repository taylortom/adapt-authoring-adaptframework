...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async installFramework() {
    try {
      ...
      this.log('debug', `local adapt_framework (${installed}) doesn't match required version (${compatible}), updating`);
  ...
  async loadSchemas() {
    ...
    await Promise.all(await AdaptCli.getSchemas(this.path).map(s => jsonschema.registerSchema(s)));
