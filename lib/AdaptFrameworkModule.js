...
class AdaptFrameworkModule extends AbstractModule {
  ...
  async installFramework() {
    try {
      ...
      if(shouldUpdate) {
        ...
        if(installed) {
          this.log('debug', `local adapt_framework v${installed} installed`);
          if(semver.lt(installed, compatible)) {
  ...
  async importCourse(importPath, userId) {
    ...
    const importer = await FrameworkImport.run({ unzipPath,  userId });
    return await AdaptFrameworkUtils.getImportSummary(importer);
  }
}
