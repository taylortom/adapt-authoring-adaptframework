...
class AdaptFrameworkImport extends Events {
  ...
  */
  ...
  async importCourseData() {
    ...
    const contentItems = await Promise.all(Object.values(this.contentJson.contentObjects).map(c => this.importContentObject(c)));
  ...
  async importContentObject(data) {
    ...
    const doc = await this.content.insert(insertData, { schemaName: AdaptFrameworkImport.typeToSchema(data._type), emitEvent: false });
