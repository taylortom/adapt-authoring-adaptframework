...
class AdaptFrameworkImport extends Events {
  ...
  */
  ...
  async importCoursePlugins() {
    ...
    results.forEach(r => r.status === 'rejected' && this.framework.log('warn', `failed to import framework plugin, ${r.reason}`));
  }
  async importCourseData() {
    const course = await this.importContentObject(this.contentJson.course);
    const coData = Object.values(this.contentJson.contentObjects);
    const contentItems = await Promise.all(coData.map(c => this.importContentObject(c)));
    this.content.emit('insert', [course, ...contentItems]);
  }
