...
class AdaptFrameworkImport extends Events {
  ...
  */
  ...
  async importCourseData() {
    ...
    this.content.emit('insert', await Promise.all([
      this.importContentObject(this.contentJson.config),
      this.importContentObject(this.contentJson.course),
      ...coData.map(c => this.importContentObject(c))
    ]));
  }
