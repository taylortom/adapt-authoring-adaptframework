...
class AdaptFrameworkUtils {
  ...
  static async handleImportFile(req, res) {
    ...
    if(req.body.url) {
      ...
      const outputDir = `${App.instance.getConfig('tempDir')}/formidable`;
  ...
  static async getHandler(req, res, next) {
    ...
    if(action === 'publish' || action === 'export') {
      ...
      return res.sendFile(buildData.location, { root: App.instance.getConfig('rootDir') });
    }
    if(action === 'preview') {
      const filePath = path.join(buildData.location, req.path.slice(req.path.indexOf(id)+id.length+1) || 'index.html');
      try {
        await fs.stat(filePath);
        res.sendFile(filePath, { root: App.instance.getConfig('rootDir') });
      } catch(ex) {
