...
class AdaptFrameworkUtils {
  ...
  /**
   * Sanitises an incoming JSON object to make sure all string values are safe from XSS-vulnerabilities
   * @param {object} data The incoming data
   * @param {object} sanitised A memo to allow the function to run recursively
   * @returns 
   */
  static sanitiseJson(data, sanitised = {}) {
    if(Array.isArray(data)) {
      return data.map(d => this.sanitiseJson(d));
    }
    const xssString = s => typeof s === 'string' ? xss(s, App.instance.dependencyloader.instances['adapt-authoring-adaptframework'].getConfig('xssWhitelist')) : s;
