...
class AdaptFrameworkUtils {
  ...
  static async getImportSummary(importer) {
    ...
    const getStatus = ([installedVersion, importVersion]) => {
      if(!installedVersion) return 'INSTALLED';
      if(semver.lt(importVersion, installedVersion)) return 'OLDER';
      if(semver.gt(importVersion, installedVersion)) return 'UPDATED';
      return 'NO_CHANGE';
    };
    ...
    return {
      ...
      versions: [{ name: fwName, versions: [fwVersion, framework.version] }].concat(
        versionsMap(usedPlugins, meta => [versionLookup(meta.name), meta.version]),
        versionsMap(newPlugins, meta => [undefined, meta.version]),
