...
class AdaptFrameworkUtils {
  ...
  static async getImportSummary(importer) {
    ...
    const getStatus = versions => semver.lte(...versions) ? 'NO_CHANGE' : 'UPDATED';
  ...
  static async importHandler(req, res, next) {
    try {
      ...
      const importer = await FrameworkImport.run({
        ...
        isDryRun: this.toBoolean(req.body.dryRun), 
        importContent: this.toBoolean(req.body.importContent), 
        importPlugins: this.toBoolean(req.body.importPlugins), 
        updatePlugins: this.toBoolean(req.body.updatePlugins)
      });
