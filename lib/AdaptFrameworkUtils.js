...
export default class AdaptFrameworkUtils {
  ...
  static async importHandler(req, res, next) {
    try {
      ...
      const summary = await AdaptFrameworkUtils.getImportSummary(await FrameworkImport.run(req));
      res.json(summary);
   ...
   * @return {Object} Object mapping all import versions to server installed versions
   * @example
   * {
  ...
  static async getHandler(req, res, next) {
    ...
    if(action === 'preview') {
      ...
      try {
        ...
        if(e.code === 'ENOENT') return next(App.instance.errors.FILE_NOT_FOUND.setData({ path: filePath }));
        return next(e);
      }
    }
  }
