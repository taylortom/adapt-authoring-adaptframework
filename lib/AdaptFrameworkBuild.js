...
import { App, Events, Hook, Utils } from 'adapt-authoring-core';
...
class AdaptFrameworkBuild extends Events {
  ...
  async build() {
    ...
    this.dir = path.resolve(App.instance.rootDir, framework.getConfig('buildDir'), new Date().getTime().toString());
    ...
    if(!this.isExport) {
      const cacheDir = path.resolve(App.instance.rootDir, framework.getConfig('buildDir'),  '.cache');
      ...
      try {
        // make sure browserslist is up to date, as will throw errors during build otherwise
        await exec('npx browserslist@latest --update-db', { cwd: this.buildDir });
        ...
        throw framework.app.errors.CLI_BUILD_FAILED
   ...
   */
   async loadAssetData() {
    ...
    await Promise.all(usedAssets.map(async a => {
      assetDocs.push(a);
      if(!idMap[a._id]) idMap[a._id] = a.url ? a.url : path.join(this.courseData.course.dir, 'assets', a.path);
    }));
  ...
  async cachePluginData() {
    ...
    const all = await contentplugin.find({}, { includeUpdateInfo: false });
  ...
  sortContentItems(items) {
    ...
    Object.entries(sorted).forEach(([type, data]) => this.courseData[type].data = Object.keys(data).sort().map(key => data[key]));
  }
  /**
  ...
  async copySource() {
    ...
    const blacklist = ['.DS_Store', 'thumbs.db', 'node_modules','course', ...this.disabledPlugins.map(p => p.name)];
