...
const { App, Events, Hook } = require('adapt-authoring-core');
...
class AdaptFrameworkBuild extends Events {
   ...
   * @property {String} data.action The type of build to execute
   * @property {String} data.courseId The course  to build
   * @property {String} data.userId The user executing the build
   * @property {String} data.expiresAt When the build expires
   * 
   * @constructor
   * @param {AdaptFrameworkBuildOptions} options
   */
  ...
  async build() {
    ...
    if(!this.isExport) {
      const devMode = !this.isPublish || this.courseData.config.data._generateSourcemap;
      await AdaptCli.buildCourse(devMode, this.dir);
    }
