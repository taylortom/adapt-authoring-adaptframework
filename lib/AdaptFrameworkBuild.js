...
class AdaptFrameworkBuild extends Events {
  ...
  static async getBuildExpiry() {
    ...
    return addToDate(amount, unit).toISOString();
  }
  /**
   * @constructor
   * @param {ClientRequest} req
  ...
  constructor(req) {
     ...
     * @type {Object}
     */
    this.expiresAt = req.expiresAt || AdaptFrameworkBuild.getBuildExpiry();
    /**
     * All JSON data describing this course
     * @type {Object}
     */
  ...
  async build() {
    ...
    this.dir = path.join(adaptFramework.getConfig('buildDir'), new Date().getTime().toString());
