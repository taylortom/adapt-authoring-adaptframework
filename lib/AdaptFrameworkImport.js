...
import { default as globCallback } from 'glob';
 ...
 * * @extends {Events}
 */
export default class AdaptFrameworkImport extends Events {
  /**
   * Runs the import
  ...
  async importCoursePlugins() {
    ...
    if(pluginNamesToInstall.length) {
      ...
      await Promise.all(pluginNamesToInstall.map(async p => {
        try {
          this.newContentPlugins[p] = await contentplugin.manualInstallPlugin(this.usedContentPlugins[p].path, { isZip: false });
        } catch(e) {
