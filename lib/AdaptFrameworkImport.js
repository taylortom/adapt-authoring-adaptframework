...
class AdaptFrameworkImport {
  ...
  async loadAssetData () {
    ...
    if (metaFiles.length) { // process included asset metadata
      await Promise.all(metaFiles.map(async f => this.assetData.push(Object.values(await fs.readJson(f)))))
    } else { // process the file metadata manually
      const assetFiles = await glob(`${this.coursePath}/*/*/*`, { absolute: true })
      this.assetData.push(...assetFiles.map(f => Object.assign({}, { title: path.basename(f), path: f })))
    }
  ...
  async importTags () {
    ...
    // add asset tags
    this.assetData.forEach(a => {
      if (a.tags) newTags.push(...a.tags.filter(t => !newTags.includes(t)))
    })
    await Promise.all(newTags.map(async t => {
      if (!existingTagMap[t.title]) {
        const { _id } = await tags.insert({ title: t })
  ...
  async importCourseAssets () {
    let imagesImported = this.settings.isDryRun ? Object.keys(this.assetData).length : 0
    await Promise.all(this.assetData.map(async data => {
      const filepath = data.path ?? (await glob(`${this.coursePath}/*/*/${data.filename}`, { absolute: true }))[0]
