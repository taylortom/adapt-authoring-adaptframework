...
class AdaptFrameworkImport {
  ...
  async loadContentFile (filePath) {
    ...
    if (!contents._type && !contents[0]._type) {
      return // not a valid content file
    }
  ...
  async importTags () {
    ...
    this.assetData.forEach(data => {
      data.tags = data.tags?.map(t => existingTagMap[t.title])
    })
  }
  ...
  getSortedData () {
    ...
    while (sortedCount < cos.length) {
      ...
      else throw App.instance.errors.FW_IMPORT_UNEXPECTED_STRUCTURE // level has no children, so something's gone wrong
    }
