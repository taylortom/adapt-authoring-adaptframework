...
class AdaptFrameworkImport extends Events {
  ...
  async importCourseAssets() {
    ...
    return Promise.allSettled(['images', 'video'].map(async assetType => {
      const files = await glob(`${this.unzipPath}/src/course/*/${assetType}/*`);
  ...
  async importContentObject(data) {
    ...
    let doc = await this.content.insert(insertData, { schemaName, emitEvent: false });
