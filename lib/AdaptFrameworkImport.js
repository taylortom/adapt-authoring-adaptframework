...
class AdaptFrameworkImport extends Events {
  ...
  async importCoursePlugins() {
    this.installedPlugins = (await this.contentplugin.find({}, { includeUpdateInfo: false })).reduce((m,p) => Object.assign(m, { [p.name]: p }), {});
    ...
    Object.keys(this.usedContentPlugins).forEach(p => {
      ...
      if(semver.lte(importVersion, installedVersion)) {
        this.statusReport.warn.push({ code: 'PLUGIN_INSTALL_NOT_NEWER', data: { name: p, installedVersion, importVersion } });
    ...
    if(pluginsToInstall.length) {
      ...
      await Promise.all([...pluginsToInstall, ...pluginsToUpdate].map(async p => {
        try {
          ...
          this.statusReport.info.push({ code: 'PLUGIN_IMPORT_SUCCESS', data: { name: p, version: pluginData.version } });
        } catch(e) {
          if(e.code !== 'EEXIST') {
            this.framework.log('error', `failed to import framework plugin, ${e}`);
      ...
      if(errors.length) {
        throw App.instance.errors.IMPORT_PLUGINS_FAILED
          .setData({ errors: errors.map(e => App.instance.lang.t(`error.${e.error.code}`, e.error.data)).join(', ') });
      }
    }
    ...
    for (let i = 0; i < sortedIds.length; i++) {
      ...
      await Promise.all(ids.map(async _id => {
        try {
          ...
          else errors.push(App.instance.lang.t(`error.${e.code}`, e.data));
        }
      }));
      if(errors.length) {
