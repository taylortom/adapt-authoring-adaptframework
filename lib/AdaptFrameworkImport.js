...
class AdaptFrameworkImport extends Events {
  ...
  async loadCourseData() {
    ...
    await Promise.all(usedPluginPaths.map(async p => {
      const { version } = await fs.readJson(`${p}/bower.json`);
      this.usedContentPlugins[p.split('/').pop()] = { path: p, version };
    }));
  ...
  async importCoursePlugins() {
    const installedPlugins = (await this.contentplugin.find()).reduce((m,p) => Object.assign(m, { [p.name]: p }), {});
    const pluginsToInstall = {};

    Object.keys(this.usedContentPlugins).forEach(p => {
      const installedP = installedPlugins[p];
    ...
    if(!this.settings.importPlugins) {
      throw new Error(`course uses the following missing plugins: ${pluginNamesToInstall.join(',')}`);
    }
    const errors = [];
    await Promise.all(pluginNamesToInstall.map(async p => {
      try {
        const { _id } = await contentplugin.manualInstallPlugin(this.usedContentPlugins[p].path, { isZip: false });
    ...
    if(errors.length) throw new Error(`failed to import framework plugins ${errors.map(e => e.plugin)}`);
  }
  /**
