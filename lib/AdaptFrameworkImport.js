...
class AdaptFrameworkImport {
  ...
  async importCourseAssets () {
    ...
    const globPath = `${basePath}/*/*`
    const metaFiles = await Promise.all((await glob(`${basePath}/*/assets.json`, { absolute: true })).map(f => fs.readJson(f)))
    const fileData = metaFiles.reduce((memo, data) => Object.assign(memo, data), {})
    // process the file metadata manually
    if (!metaFiles.length) {
      const files = await glob(`${globPath}/*`, { absolute: true })
      files.forEach(f => {
        const filename = path.basename(f)
        fileData[filename] = { title: filename, description: filename, path: f }
      })
    }
    let imagesImported = this.settings.isDryRun ? Object.keys(fileData).length : 0
    await Promise.all(Object.entries(fileData).map(async ([filename, data]) => {
      const filepath = data.path ?? (await glob(`${globPath}/${filename}`, { absolute: true }))[0]
      if (this.settings.isDryRun) {
        return
      }
      // add any global tags
      if (this.tags.length) data.tags = (data.tags ?? []).push(...this.tags)
      ...
      try {
        const asset = await this.assets.insert({
          ...
          tags: data.tags
            ? (await Promise.all(data.tags.map(async t => {
                return (await tags.insert({ title: t.title }))._id.toString()
              })))
            : []
        })
        // store the asset _id so we can map it to the old path later
        const resolved = path.relative(basePath + '/..', filepath).replaceAll(path.sep, '/')
  ...
  async importCourseData () {
    ...
    await Promise.all([course, config].map(({ _id, _type, _courseId }) => {
      return this.importContentObject({ ...this.contentJson[_type], _id, _courseId }, { isUpdate: true })
    }))
  ...
  async importContentObject (data, options = {}) {
    ...
    const schema = await this.content.getSchema(schemaName, data)
    try {
      this.extractAssets(schema.built.properties, data)
    } catch (e) {
    ...
    // sanitise and transform
    insertData = await this.transformData(await schema.sanitise(data))
    let doc
    const opts = { schemaName, updateEnabledPlugins: false, validate: true, useCache: false }
    if (options.isUpdate) {
  ...
  async transformData (data) {
    await Promise.all(ContentMigrations.map(Migration => Migration(data, this)))
    return data
  }
