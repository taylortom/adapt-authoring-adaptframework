...
import ContentObjectTransform from './migrations/contentobject.js';
  ...
  async importCoursePlugins() {
    ...
    if(pluginNamesToInstall.length) {
      ...
      await Promise.all(pluginNamesToInstall.map(async p => {
        try {
          this.newContentPlugins[p] = await this.contentplugin.installPlugins([this.usedContentPlugins[p].path]);
        } catch(e) {
          if(e.code !== 'EEXIST') {
