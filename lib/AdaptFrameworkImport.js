...
class AdaptFrameworkImport extends Events {
  ...
  async importCoursePlugins() {
    ...
    const installedPlugins = (await contentplugin.find()).map(p => p.name);
    const pluginsToInstall = Object.keys(this.contentPlugins).filter(p => !installedPlugins.includes(p));
    const results = await Promise.allSettled(pluginsToInstall.map(p => {
      return contentplugin.manualInstallPlugin(this.contentPlugins[p], { isZip: false });
    }));
    results.forEach(r => {
