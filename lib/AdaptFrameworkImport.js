...
class AdaptFrameworkImport extends Events {
  ...
  async importContentObject(data) {
    ...
    const doc = await this.content.insert(insertData, { schemaName, emitEvent: false });
    this.idMap[data._id] = doc._id.toString();
    if(doc._type === 'course') {
      const _courseId = this.idMap.course;
      await this.content.update({ _id: _courseId }, { _courseId }, { emitEvent: false, schemaName, validate: false });
    }
