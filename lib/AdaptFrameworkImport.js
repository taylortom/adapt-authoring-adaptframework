...
class AdaptFrameworkImport {
    ...
    await Promise.all(Object.entries(fileData).map(async ([filename, data]) => {
      ...
      try {
        const asset = await this.assets.insert({ 
          ...
          tags: data.tags ? (await Promise.all(data.tags.map(async t => {
        ...
        this.statusReport.warn.push({ code: 'ASSET_IMPORT_FAILED', data: { filepath: filepath } });
      }
      imagesImported++;
    }));
  ...
  async importCourseData() {
    ...
    if(errors.length) throw App.instance.errors.IMPORT_CONTENT_FAILED.setData({ errors: errors.join('; ') });
  }
  /**
  ...
  async importContentObject(data, options = {}) {
    ...
    let doc;
    const opts = { schemaName, /*updateSortOrder: false,*/ updateEnabledPlugins: false, validate: true, useCache: false };
    if(options.isUpdate) {
