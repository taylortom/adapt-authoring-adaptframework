...
class AdaptFrameworkImport {
  ...
  async loadAssetData () {
    this.assetData = []
    const metaFiles = await glob(`${this.coursePath}/*/assets.json`)
    if (metaFiles.length) { // process included asset metadata
      ...
      const assetFiles = await glob(`${this.coursePath}/*/*/*`)
  ...
  async loadCourseData () {
    const usedPluginPaths = await glob(`${this.unzipPath}/src/+(components|extensions|menu|theme)/*`)
  ...
  async importCourseAssets () {
    ...
    await Promise.all(this.assetData.map(async data => {
      const filepath = data.filepath ?? (await glob(`${this.coursePath}/*/*/${data.filename}`))[0]
