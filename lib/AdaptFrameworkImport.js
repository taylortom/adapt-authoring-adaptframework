...
class AdaptFrameworkImport extends Events {
  ...
  async import() {
    ...
    try {
      ...
      await this.prepare();
      this.log('debug', 'preparation tasks completed successfully');
      
      if(this.settings.importContent) {
        await this.importCourseAssets();
        this.log('debug', 'imported course assets successfully');
      }
      await this.loadCourseData();
      this.log('debug', 'loaded course data successfully');
      
      if(this.settings.importPlugins) {
        await this.importCoursePlugins();
        this.log('debug', 'imported course plugins successfully');
      }
      if(this.settings.importContent) {
        await this.importCourseData();
        this.log('debug', 'imported course data successfully');
      }
    } catch(e) {
      error = e;
    }
  ...
  async convertSchemas() {
    return octopus.runRecursive({ cwd: this.unzipPath });
  }
  /**
  ...
  async importCourseData() {
    ...
    const newCos = [];
    const sortedIds = await this.getSortedIds().slice(1); // remove the top-level course, as we've already created that
    ...
    for (let i = 0; i < sortedIds.length; i++) {
      ...
      await Promise.all(ids.map(async _id => {
        try {
          newCos.push(await this.importContentObject(this.contentJson.contentObjects[_id]));
        } catch(e) {
     ...
     */
    await Promise.all(newCos.map(async c => await this.content.updateSortOrder(c)));
