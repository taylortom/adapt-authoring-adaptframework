...
class AdaptFrameworkBuild extends Events {
  ...
  */
  ...
  async loadCourseData() {
    ...
    const contentItems = await content.find({ _courseId: mongodb.ObjectId.parse(course._id) });
    contentItems.forEach(this.groupContentItem.bind(this));
  }
  groupContentItem(contentItem) {
    if(contentItem._type === 'config') {
      this.courseData.config.data = contentItem;
    } else if(contentItem._type === 'menu' || contentItem._type === 'page') {
      this.courseData.contentObject.data.push(contentItem);
    } else {
      this.courseData[contentItem._type].data.push(contentItem);
    }
  }
  async copySource() {
